...
class PowerOff(object):
    def __init__(self, poweroff_which):
        self.poweroff_which = poweroff_which
        self.load_gladexml()

        # Keep it stayed at the top of the parent window dialog, until closing
        self.msgdia_poweroff.set_modal(True)
        # self.msgdia_poweroff.set_decorated(False)

        self.msgdia_poweroff.set_title("")
#        self.btn_ok = self.msgdia_poweroff.add_button("OK", gtk.RESPONSE_OK)
#        self.btn_cancel = self.msgdia_poweroff.add_button("Cancel", gtk.RESPONSE_CANCEL)
    ...
    def load_gladexml(self):
        try:
            ...
            self.builder.add_from_file("poweroffdialog.glade")
        except:
            log.error("Failed to load UI XML fle: poweroffdialog.glade")
            sys.exit(1)
        ...
        if self.poweroff_which == 'tech_and_stu':
            self.label.set_text("确认关闭学生端和教师端？")
        if self.poweroff_which == 'server':
            self.label.set_text("是否同时关闭服务器？")
